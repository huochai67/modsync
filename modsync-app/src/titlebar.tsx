    ...
    return (
        <div data-tauri-drag-region className="flex items-center h-full border-x-4 border-background text-foreground bg-background">
            ...
            <ButtonGroup variant="light" size="sm">
                ...
                <Button isIconOnly aria-label="tb-minus" onClick={() => { getCurrentWindow().minimize() }}><MinusIcon className="size-6" /></Button>
                <Button isIconOnly aria-label="tb-close" onClick={() => { getCurrentWindow().close() }}><XMarkIcon className="size-6" /></Button>
            </ButtonGroup>
        </div>
    )
}
