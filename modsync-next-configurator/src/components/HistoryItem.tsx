        ...
        <div className="history-item rounded-lg shadow-sm border overflow-hidden mb-4 transition-all">
            ...
            >
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h3 className="font-semibold text-sm truncate max-w-xs">{release.changelog}</h3>
                        <p className="text-muted text-xs">{release.date}</p>
                    </div>
                </div>

                <div className="flex items-center space-x-2">
                    {release.adds && release.adds.length > 0 && <Chip color='success'>+{release.adds.length}</Chip>}
                    {release.subs && release.subs.length > 0 && <Chip color='danger'>-{release.subs.length}</Chip>}
                    {release.mods && release.mods.length > 0 && <Chip color='accent'>*{release.mods.length}</Chip>}
                    <span className="text-muted text-xs ml-2">{formatBytes(release.size)}</span>
                    <svg
                        ...
                        {release.adds && release.adds.length > 0 && (
                            <div>
                                <h4 className="font-bold mb-1 flex items-center">
                                    ...
                                    <span className="text-[10px] bg-success-hover px-1 rounded">{release.adds.length}</span>
                                </h4>
                        ...
                        {release.mods && release.mods.length > 0 && (
                            <div>
                                <h4 className="font-bold mb-1 flex items-center">
                                    ...
                                    <span className="text-[10px] bg-accent-hover px-1 rounded">{release.mods.length}</span>
                                </h4>
                                <ul className="text-muted space-y-1">
                        ...
                        {release.subs && release.subs.length > 0 && (
                            <div>
                                <h4 className="font-bold mb-1 flex items-center">
                                    ...
                                    <span className="text-[10px] bg-danger-hover px-1 rounded">{release.subs.length}</span>
                                </h4>
                                <ul className="text-muted space-y-1">
